...
def indexPage(request):
    ...
    if request.method == "POST":
        ...
        if request.FILES.get("input_file"):   ### request.FILES in this case because input is a file, if input_file is not empty, run this
            ...
            final_downloads['LDA'] = LDA_output.drop(columns = ['tokenized_words']).to_json()
            final_downloads['Topwords'] = TOPWORDS.reset_index().to_json()
            final_downloads['TFIDF'] = TFIDF.to_json()
            final_downloads['Bi-Grams'] = BIGRAMS.reset_index().rename(columns = {0:"word_counts","index":"bigram"}).to_json()
            final_downloads['Stopwords'] = pd.read_csv(os.path.join(os.path.dirname(settings.BASE_DIR), 'osprey_admin/analysis_builds/stopwords.csv')).to_json()
            ...
            'download_dictionary':final_downloads,
        ...
        # Normal GET Request (most likely). We should probably display the form, so it can be filled out by the user and submitted. 
